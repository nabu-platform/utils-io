Because the first question to ask with all libraries is: how easy is to use; this readme starts with two small examples:

Simple Example
--------------
This simple example was pulled from the unit test for FileWrapper, comments were added for this readme:
...
-----------------------------
This example was pulled from a testcase for quoted printable encoding, I added some comments:
...
The last bit can also be reversed: you can decode on read instead of on write. There is however one tricky detail to get right: a write uses the flush (or the close which always flushes) to indicate that you are done
This will trigger the generation of lead-out data.
The read however only knows that the data stream is done when it is closed which means we really need to close the decoded container here:
...
The are a couple of problems with I/O in java, at the core is that there are two fundamentally different ways of doing I/O but no general way to work with both.
Additionally both implementations use abstract base classes which can complicate implementations that try to work with both.
As an example suppose you want to do base64 encoding, if you write your encoder with both I/O methods in mind, it is still hard to write something that works for both.
...
Next let's take Channel, it is a lightweight interface around Closeable and offers little of value, the interesting interfaces are:
...
because Buffer does not allow generic read/write, only the specific implementations (ByteBuffer et al) offer this.
...
The design was based on (of course) the standard java library but also on for example jboss netty which has its own buffer-stack.
...
Peekable allows you to peek content from a container without actually updating the internal state. There needed to be a way to get content from a container without actually altering that container. 
...
To Do
-----
The flush() in WritableContainer should return a long which should indicate if everything was already flushed (0) or it actually flushed something.
