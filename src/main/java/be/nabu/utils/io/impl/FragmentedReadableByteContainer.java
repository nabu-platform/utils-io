 ...
 */
public class FragmentedReadableByteContainer implements LimitedReadableByteContainer, SkippableByteContainer, MarkableByteContainer, PeekableByteContainer {
	...
	List<byte[]> backingArrays = new ArrayList<byte[]>();

	FragmentedReadableByteContainer(List<byte[]> backingArrays, int size, int writeIndex, int writeOffset, int readIndex, int readOffset) {
		// make a copy of the original list so if we "release" data from the list, we don't alter the original list
		this.backingArrays = new ArrayList<byte[]>(backingArrays);
		this.size = size;
	...
	private FragmentedReadableByteContainer parent;
	
	private FragmentedReadableByteContainer(FragmentedReadableByteContainer parent) {
		this.parent = parent;
	...
	/**
	 * Createsa new readable which is based on the marked position of this container
	 */
	@Override
	public FragmentedReadableByteContainer clone() {
		if (releaseRead)
			throw new IllegalStateException("No mark has been set, can not clone");
		return new FragmentedReadableByteContainer(this);
	}
	...
	public int peak(byte[] bytes, int offset, int length) {
		...
		// perform a read
		int amountRead = read(bytes, offset, length);
